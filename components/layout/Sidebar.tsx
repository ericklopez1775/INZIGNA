...
export default function Sidebar({ user }: SidebarProps) {
  ...
  return (
    <aside className="w-64 h-screen flex flex-col glass border-r border-white/8 fixed left-0 top-0 z-40">
      {/* Logo */}
        ...
        {visible.map((item) => {
          ...
          return (
            <Link
              ...
              `}
            >
              <span className={isActive ? "text-brand-red" : "group-hover:text-white/80"}>
                {item.icon}
              </span>
              {item.label}
      ...
      <div className="p-4 border-t border-white/8">
        ...
        <form action="/api/auth/logout" method="POST">
          <button
            type="submit"
            className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-white/50 hover:text-white hover:bg-white/6 transition-colors"
          >
            <LogOut size={16} />
            Cerrar sesión
          </button>
